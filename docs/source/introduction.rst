...
例如函数 `λx. f x` 代表函数 `x ↦ f x`，如果 `x` 是类型 `s1` 的元素， `f x` 是类型 `s2` 的元素，那么函数 `x ↦ f x` 就是类型为
`s1 → s2` 的元素。可以在这个类型论中加入更多的符号用来表达更复杂的数学对象，例如用 `∧` 表示且命题，用 `¬` 表示非命题，用 `∀` 表示全称量词等等。
那么数学归纳法就可以被表达为 `∀(λP.P(0) ∧ ∀(λn.P(n) ⇒ P(S(n))) ⇒ ∀(λn.P(n)))`。
